...
must(index.includes('arena-commerce-v1.js?v=20260825-06'),'commerce bridge is not loaded');
must(config.includes("mode:'native-bridge-server-authority'"),'commerce mode missing');
must(config.includes("premium:'arena_premium_monthly'"),'Premium product ID is not canonical');
must(config.includes("pro:'arena_pro_monthly'"),'Pro product ID is not canonical');
must(sw.includes("'./arena-commerce-v1.js'"),'commerce bridge missing from offline shell');
must(sw.includes('arena-edu-core-3.8.0-build-20260825-06'),'cache version mismatch');
must(manifest.includes('id=20260825-06'),'manifest build ID mismatch');
