(() => {
  ...
  function normalize(state=read()){
    ...
    copy.demoPlan=validPlan(copy.demoPlan)||validPlan(copy.plan)||(legacy?'premium':'free');
    copy.plan='free';
    copy.isPremium=false;
    return copy;
  }
  function persist(state){localStorage.setItem(ARENA_KEY,JSON.stringify(normalize(state)))}
  function current(){return window.ArenaSecurityV1?.active?.()?window.ArenaSecurityV1.entitlement().plan:'free'}
  function demoCurrent(){return normalize().demoPlan}
  // Legacy migration marker only: freePremiumVerifiedByServer===true is never authoritative.
  function giftActive(){return false}
  function atLeast(plan){return !!window.ArenaSecurityV1?.atLeast?.(plan)}
  ...
  async function exportBackup(){
    ...
    const payload=JSON.stringify(data),file={format:'LGS_ARENA_DEVICE_TRANSFER',schema:1,createdAt:new Date().toISOString(),buildId:'20260824-26',automaticSync:false,data,checksum:await checksum(payload)};
  ...
  function refresh(){
    const plan=demoCurrent(),status=document.getElementById('membershipStatus');if(status){status.textContent=LABELS[plan].toLocaleUpperCase('tr-TR')+' · DEMO';status.classList.toggle('active',atLeast('premium'))}
  ...
  window.LgsArenaPlans=Object.freeze({current,demoCurrent,label:()=>LABELS[current()],atLeast,can,setPlan,normalize,exportBackup,importBackup,features:FEATURES,automaticSync:false,syncMode:'manual-device-transfer',recommendedBackend:'firebase-spark'});
