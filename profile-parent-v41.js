(() => {
  ...
  function refreshChip(){const profile=readProfile(),chip=byId('studentProfileChip'),name=nameOf(profile);if(chip){chip.title=`${name} · Hesap ve ayarlar`;chip.setAttribute('aria-label',`${name} hesap ve ayarlarını aç`)}}
  ...
  function installUi(){
    ...
    const chip=byId('studentProfileChip');if(chip)chip.onclick=()=>{if(window.ArenaAccountBridge?.openSettings)window.ArenaAccountBridge.openSettings();else{renderProfile();open('profileOverlay')}};
